...
int LoadAssignmentCorpusWithRule(const string& Assignment_file,
                              int num_topics,
                              LDACorpus* corpus,
                              LDAModel* new_model,
                              map<string, int>* word_index_map) {
...
//   }
  ...
  LDAModel new_model(model.num_topics() + flags.new_topic_num_, word_index_map);
  ...
  std::cout << "new_topic_num " << flags.new_topic_num_ << "\n";
  ...
  sampler.AdjustCorpusWithRule(corpus,flags.new_topic_num_ ,new_words,rule);
