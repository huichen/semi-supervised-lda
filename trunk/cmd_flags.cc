...
LDACmdLineFlags::LDACmdLineFlags() {
  ...
  file_type_ = 0;
  save_step_ = 100;
  topic_distribution_file_ = "";
  word_index_file_ = "";
  topic_assignments_file_ = "";
...
void LDACmdLineFlags::ParseCmdFlags(int argc, char** argv) {
  for (int i = 1; i < argc; ++i) {
    if (0 == strcmp(argv[i], "--num_topics")) {
      ...
      ++i;
    }else if (0 == strcmp(argv[i], "--new_model_file")) {
      new_model_file_ = argv[i+1];
      ++i;
    }else if (0 == strcmp(argv[i], "--new_word_file")) {
      new_words_file_ = argv[i+1];
      ++i;
    }else if (0 == strcmp(argv[i], "--rule_file")) {
      rule_file_ = argv[i+1];
      ++i;
    }else if (0 == strcmp(argv[i], "--topic_distribution_file")) {
      topic_distribution_file_ = argv[i+1];
      ++i;
    }else if (0 == strcmp(argv[i], "--word_index_file")) {
      word_index_file_ = argv[i+1];
      ++i;
    }else if (0 == strcmp(argv[i], "--topic_assignments_file")) {
      topic_assignments_file_ = argv[i+1];
      ++i;
    }else if (0 == strcmp(argv[i], "--file_type")) {
      std::istringstream(argv[i+1]) >> file_type_;
      ++i;
    }else if (0 == strcmp(argv[i], "--save_step")) {
      std::istringstream(argv[i+1]) >> save_step_;
      ++i;
      ...
      ++i;
    } else if (0 == strcmp(argv[i], "--new_topic_num")) {
      std::istringstream(argv[i+1]) >> new_topic_num_;
      ++i;
