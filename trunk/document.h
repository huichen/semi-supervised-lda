...
class LDADocument {
 public:
  ...
  LDADocument(const string& doc_name,const DocumentWordTopicsPB& topics, int num_topics);
  ...
  const int document_size() const {
    return document_size_;
  }

  const DocumentWordTopicsPB * topic_assignments() const {
    return topic_assignments_;
  }
  ...
  int word_assignments(vector<pair<int,int> >& word_assign);

//  void ResetWordIndex(const map<string, int>& word_index_map);
  void resize_topic_distribution(int num);
