...
vector<int32> LDADocument::WordOccurrenceIterator::GetTopicList() {
  CHECK(!Done());
  return parent_->topic_assignments_->wordtopicslist(word_topic_index_);
}
...
void LDADocument::WordOccurrenceIterator::SetTopicList(vector<int32> & topiclist) {
  parent_->topic_assignments_->wordtopicslist_[word_topic_index_]=topiclist;
}
...
//string LDADocument::WordOccurrenceIterator::WordStr() {
//  CHECK(!Done());
//  return parent_->topic_assignments_->wordstr(word_index_);
//}
...
LDADocument::LDADocument(const string& doc_name,const DocumentWordTopicsPB& topics,
                         int num_topics) {
  document_name_=doc_name;
  freeze_ = false;
  document_size_=topics.document_size();
...
int LDADocument::word_assignments(vector<pair<int,int> >& word_assign){
  word_assign.clear();
  for (WordOccurrenceIterator iter(this); !iter.Done(); iter.Next()) {
    word_assign.push_back(make_pair(iter.Word(),iter.Topic()));
  }
  return word_assign.size();
}

void LDADocument::resize_topic_distribution(int num){
  for(int i = topic_distribution_.size() ; i < num ; i++)
  {
    topic_distribution_.push_back(0);
  }
}
