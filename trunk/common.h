...
//  vector<string> words_s_;
  ...
  vector < vector<int32> > wordtopicslist_;
  ...
  int document_size() const { return wordtopics_.size(); }
...
//  string wordstr(int word_index) const { return words_s_[word_index]; }
  int32 wordtopics(int index) const { return wordtopics_[index]; }
  vector<int32> wordtopicslist(int index) const { return wordtopicslist_[index]; }
